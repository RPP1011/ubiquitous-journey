...
// Drives the surveil→approach→burgle heist end-to-end through the REAL frame loop (the urchin
// feature is always-live on the mainline). Asserts: (D) a poor + larcenous agent DERIVES a steal
// goal against a believed mark; (E) the heist executes — gold is CONSERVED (the take debits the
// mark, never mints) and the consequence EMERGES (the mark sours / grows suspicious of the thief).
...
export function urchinTest(ok, helpers) {
  {
    ...
    // value with a confidence, read off belief cues, not a flat constant (always-live on the mainline).
    {
      const st = new FeatureStage(helpers);
      const thief = st.add('Pip', 0, 0, { personality: { altruism: 0.2, risk_tolerance: 0.8 } });
      ...
      ok(!!g && Math.round(g.target) === Math.round(estimateHaul(thief, rich.id).value),
        `urchin W4: the heist aims for the ESTIMATED haul (target=${g ? g.target : 'n/a'})`);
      st.dispose();
    }
  }
}
