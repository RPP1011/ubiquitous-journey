// ---- outcome-conditioned caution, gated (docs/architecture/11) ---------------------------------
...
// the §8 regression that an unwatched combat verb is never burned. Restores CAUTION in `finally`.
...
import { CAUTION, URCHIN, ROB, QUANTITY } from '../../js/sim/simconfig.js';

export function cautionTest(ok, helpers) {
  const prev = CAUTION.enabled;
  ...
  try {
    ...
    // C12 — BOUNDS & byte-identity. |s| clamps; the store evicts past maxKeys; OFF ⇒ no writes/reads.
    {
      const a = mk();
      ...
      const wasOff = CAUTION.enabled; CAUTION.enabled = false;
      const off = feltSurcharge(a, 'burgle', {}, now);
      CAUTION.enabled = wasOff;
      ok(off === 0, 'C12: feltSurcharge is 0 when the flag is OFF (byte-identical reads)');
    }
    ...
    {
      const prevU = URCHIN.enabled, prevR = ROB.enabled, prevQ = QUANTITY.enabled;
      URCHIN.enabled = true; ROB.enabled = true; QUANTITY.enabled = true;
      try {
        ...
        st.dispose();
      } finally { URCHIN.enabled = prevU; ROB.enabled = prevR; QUANTITY.enabled = prevQ; }
    }
  } finally { CAUTION.enabled = prev; }
}
