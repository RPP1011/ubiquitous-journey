...
| 15 (LLD) | [Narrative ability generation](15-ability-generation-lld.md) | **design — to build, v2**: abilities born from STORY (event-grants off the memory/oath/arc/faith folds, with provenance the biography reads), making story when used (drama-layer ops: rumour/bless/curse/denounce/oathbind; story-state conditions — vs_sworn_foe, while_faithful), known about (`believedProwess` gossip with teeth), teachable via the recipe machinery (House techniques that can die out); the v1 budgeted clause grammar demoted to assembly machinery; rule R1 — an op ships only with its consumer |
