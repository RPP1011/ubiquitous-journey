...
// rows. ALWAYS-LIVE on the mainline.
//
...
// (cheap, vaguer), study (taught — a recipe, paying conserved tuition to a co-located teacher).
// Conserved-safe: knowledge writes touch no gold beyond that tuition transfer.
...
function accrueTopic(a: Agent, topic: KnowTopic | undefined, gain: number, now: number): void {
  ...
  switch (topic.kind) {
    case 'recipe':
      learnRecipe(a, topic.good, gain, 0, now);                      // graded conf (§6) ⇒ half-learned firms up
      break;
    case 'whereabouts': {
...
// the recipe. The session pays TUITION to a co-located teacher (a conserved transfer,
// resolver.teachRecipe) and adds graded confidence (§6) — so a recipe takes a few taught sessions
// and there is no free lunch (no teacher present ⇒ no taught learning, the apprentice waits /
// re-plans).
...
// craftable Set. Own-state only; ALWAYS-LIVE on the mainline.
registerDeriver((a: Agent, ctx: CognitionCtx | null) => {
  if (!a || !a.alive) return;
