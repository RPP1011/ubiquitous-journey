...
//
// Day-one OFF (RECIPES.graded false): every function falls back to the binary Set behaviour, so the
// soak is byte-identical. Own-state only (cognition-safe); guarded; never throws on the tick.
...
// believed confidence in a recipe (0..1). Graded ⇒ the map (with a 1.0 fallback for a recipe that
// is in the seeded Set but not yet mirrored into the map); binary ⇒ present-in-Set is fully sure.
export function recipeConf(a: Agent, good: string | null | undefined): number {
  if (!a || !good) return 0;
  if (!RECIPES.graded) return a.recipes && a.recipes.has(good) ? 1 : 0;
...
// is the provenance of the channel (0 = taught/first-hand, higher = vaguer). Binary ⇒ a plain add.
export function learnRecipe(a: Agent, good: string | null | undefined, gain: number, hops: number, now: number): void {
  if (!a || !good) return;
  if (!RECIPES.graded) { if (a.recipes) a.recipes.add(good); return; }
...
// per-cognition-tick maintenance (graded only): lazy-seed the map from a producer's seeded Set,
// keep the agent's PRACTISED craft (`tradeGood`) fresh (a working master never forgets its own
// trade), and FORGET every other recipe a little — the use-it-or-lose-it fade. Sub-threshold goods
// drop out of the craftable Set; fully-faded ones are pruned. Own-state; bounded by the recipe set.
export function forgetTick(a: Agent, now: number, tradeGood: string | null | undefined): void {
  if (!RECIPES.graded || !a || !a.recipes) return;
  if (!a._recipeKnow) a._recipeKnow = new Map();
