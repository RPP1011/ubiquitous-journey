...
function carveMaze(S) {
  const g = Array.from({ length: S }, () => new Array(S).fill(1)); // 1=wall 0=floor
  const inb = (x, y) => x > 0 && y > 0 && x < S - 1 && y < S - 1;
  const stack = [[1, 1]];
  g[1][1] = 0;
...
function openRoomsAndLoops(g, S) {
...
function farthestFloor(g, S, sx, sy) {
  const dist = Array.from({ length: S }, () => new Array(S).fill(-1));
  const q = [[sx, sy]];
  dist[sy][sx] = 0;
  let far = [sx, sy], fd = 0;
  while (q.length) {
...
export class Dungeon {
  ...
  // --- coordinate helpers ---------------------------------------------------
  cellToWorld(i, j) {
    return new THREE.Vector3((i - this.half) * this.tile, this.y, (j - this.half) * this.tile);
  }
  _cellIndex(x, z) {
  ...
  walkableAt(x, z) { const [i, j] = this._cellIndex(x, z); return this.isFloor(i, j); }
  ...
  // --- geometry -------------------------------------------------------------
  _mat(opts) { const m = new THREE.MeshStandardMaterial(opts); this._materials.push(m); return m; }
  ...
  _build() {
    ...
    const amb = new THREE.HemisphereLight(0x3a4055, 0x100c08, 0.35);
  ...
  _buildMarkers() {
    ...
    const steps = new THREE.Mesh(new THREE.BoxGeometry(this.tile * 0.7, 0.5, this.tile * 0.7), stepMat);
    ...
    const glow = new THREE.PointLight(0xffcc55, this.hasRelic ? 1.4 : 0.7, 6);
  ...
  _padMesh(color) {
    ...
    const ring = new THREE.Mesh(new THREE.RingGeometry(this.tile * 0.28, this.tile * 0.42, 28), mat);
    ring.rotation.x = -Math.PI / 2;
    return ring;
  }

  _buildTorches(wallCells) {
    ...
    for (let n = 0; n < take; n++) {
      ...
      const light = new THREE.PointLight(0xff8a32, 1.6, this.tile * 4.5, 1.6);
      light.position.set(p.x, this.y + DUNGEON.wallH * 0.62, p.z);
      this.group.add(light);
      this._torchLights.push({ light, base: 1.6, phase: rng() * TAU });
      const sconce = new THREE.Mesh(new THREE.SphereGeometry(0.12, 6, 5), sconceMat);
