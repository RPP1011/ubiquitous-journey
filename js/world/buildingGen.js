...
function mat(color, flat) {
  return new THREE.MeshStandardMaterial({ color, roughness: 1, flatShading: !!flat });
}
...
export function generateBuilding(rng, params) {
  const g = new THREE.Group();
  const p = params || {};
  const kind = p.kind === 'tavern' ? 'tavern' : 'home';
  const wealth = Math.max(0, +p.wealth || 0);
  const fp = p.footprint || { w: 4, d: 4 };
  const baseW = Math.max(2, +fp.w || 4);
  const baseD = Math.max(2, +fp.d || 4);
  const storeys = Math.max(1, Math.min(3, (p.storeys | 0) || 1));
  ...
  if (wealth >= 1) {
    ...
    const course = new THREE.Mesh(
      new THREE.BoxGeometry(baseW + 0.3, ch, baseD + 0.3), mat(0x8a8276, true));
  ...
  for (let s = 0; s < storeys; s++) {
    ...
    const wall = new THREE.Mesh(
      new THREE.BoxGeometry(w, storeyH, d),
  ...
  if (gable) {
    ...
    for (const side of [-1, 1]) {
      const plank = new THREE.Mesh(
        new THREE.BoxGeometry(len, 0.18, topD + 0.4), mat(roofCol, true));
    ...
    const ridge = new THREE.Mesh(
      new THREE.BoxGeometry(0.22, 0.22, topD + 0.4), mat(tint(roofCol, 0.85), true));
    ...
    const cone = new THREE.Mesh(new THREE.ConeGeometry(rr, rh, 4), mat(roofCol, true));
  ...
  const door = new THREE.Mesh(new THREE.BoxGeometry(0.12, doorH, doorW), doorMat);
  ...
  for (let s = 0; s < storeys; s++) {
    ...
    for (let i = 0; i < winPerStorey; i++) {
      ...
      const win = new THREE.Mesh(new THREE.BoxGeometry(0.08, ws, ws), glass);
      if (onFace) {
      ...
      if (wealth >= 1 && rng() < 0.6) {
        for (const off of [-1, 1]) {
          const sh = new THREE.Mesh(new THREE.BoxGeometry(0.06, ws, ws * 0.3), mat(tint(pick(rng, pal), 0.8)));
  ...
  if (storeys >= 2 && wealth >= 2 && rng() < 0.7) {
    const deck = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.08, baseD * 0.6), mat(0x6a4b2f));
    deck.position.set(baseW / 2 + 0.25, groundY + storeyH + 0.1, 0);
    g.add(deck);
    for (const off of [-1, 1]) {
      const post = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.6, 6), mat(0x6a4b2f));
  ...
  for (let c = 0; c < chimneys; c++) {
    ...
    const stack = new THREE.Mesh(new THREE.BoxGeometry(cw, ch, cw), mat(0x6f5a4a, true));
    stack.position.set(cx, eaveY + ch * 0.5, cz);
    stack.castShadow = true; g.add(stack);
    const cowl = new THREE.Mesh(new THREE.ConeGeometry(cw * 0.6, cw * 0.6, 5), mat(0x4a3a30, true));
  ...
  if (kind === 'tavern') {
    ...
    const arm = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.9, 6), mat(0x4a3526));
    arm.rotation.z = Math.PI / 2;
    arm.position.set(baseW / 2 + 0.45, groundY + storeyH * 0.85, baseD * 0.35);
    g.add(arm);
    const board = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.6, 0.7), mat(pick(rng, [0x7a3a2a, 0x3a5a7a, 0x5a4632])));
    board.position.set(baseW / 2 + 0.85, groundY + storeyH * 0.85 - 0.35, baseD * 0.35);
    g.add(board);
    // a couple of barrels flanking the door.
    for (const off of [-1, 1]) {
      const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.26, 0.7, 10), mat(0x6a4b2f));
    ...
    const light = new THREE.PointLight(0xff9a40, 5, 10, 2);
